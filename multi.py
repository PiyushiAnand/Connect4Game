...
def draw_board(screen, board, score1, score2, current_turn):
    font = pygame.font.Font(None, 36)
    ...
    text_surface1 = font.render(text1, True, RED)
    ...
    text_surface2 = font.render(text2, True, YELLOW)
    ...
    current_turn_text = f"Current Turn: {'Player 1 (RED)' if current_turn == 1 else 'Player 2 (YELLOW)'}"
    current_turn_surface = font.render(current_turn_text, True, WHITE)
    current_turn_x = (WINDOW_WIDTH - current_turn_surface.get_width()) // 2
    current_turn_y = 10
    screen.blit(current_turn_surface, (current_turn_x, current_turn_y))

def drop_piece(screen, board, row, col, piece, score1, score2, current_turn):
    if row is not None:
        falling_piece_y = 0
        ...
        while falling_piece_y < final_y:
            screen.fill(BLACK)
            draw_board(screen, board, score1, score2, current_turn)
            ...
            if data:
                if len(data) == 2:
                    ...
                    if n_r is not None and n_c is not None:
                        drop_piece(screen, board, n_r, n_c, 3 - turn, score1, score2, current_turn)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and current_turn == turn:
                ...
                if is_valid(board, col):
                    ...
                    drop_piece(screen, board, row, col, turn, score1, score2, current_turn)
...
def main():
    ...
    screen.fill(BLACK)
    play_button = pygame.Rect((WINDOW_WIDTH - BUTTON_WIDTH) // 2 - 200, 300, BUTTON_WIDTH, BUTTON_HEIGHT)
    connect_button = pygame.Rect((WINDOW_WIDTH - BUTTON_WIDTH) // 2 + 200, 300, BUTTON_WIDTH, BUTTON_HEIGHT)
